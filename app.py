...
with e.sidebar:
    ...
    e.write("https://malleable-verdict-0bb.notion.site/a943a7db7654827b882a01be2d3c83d2?pvs=105")
    
    # 기본값으로 샘플 구글 폼 주소 지정 (사용자가 본인의 실제 폼 URL로 변경 가능)
    form_url = e.text_input(
        "건의사항 폼(Form) URL",
        value="https://malleable-verdict-0bb.notion.site/a943a7db7654827b882a01be2d3c83d2?pvs=105"
    )
...
with e.expander("❓ 정말 익명이 보장되나요?"):
    e.write("""
    네, 안심하셔도 좋습니다! 저희 스마트 건의함이 사용하는 시스템은 제출자의 계정 정보 수집하지 않도록 설정되어 있습니다.  
    개인정보 침해 걱정 없이 자유로운 의견을 남겨주세요!!
    """)

with e.expander("❓ 처리 결과는 어떻게 확인하나요?"):
    e.write("""
    검토 및 처리가 완료된 건의사항은 폼 상단 혹은 공지방을 통해 정기적으로 업데이트될 예정입니다.
    """)
